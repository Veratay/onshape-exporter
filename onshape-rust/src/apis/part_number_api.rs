...
/// struct for typed errors of method [`update_next_numbers`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum UpdateNextNumbersError {
    DefaultResponse(std::collections::HashMap<String, Vec<models::BtNextPartNumber>>),
    UnknownValue(serde_json::Value),
}


pub async fn update_next_numbers(configuration: &configuration::Configuration, bt_next_part_numbers_param: models::BtNextPartNumbersParam, cid: Option<&str>, did: Option<&str>) -> Result<std::collections::HashMap<String, Vec<models::BtNextPartNumber>>, Error<UpdateNextNumbersError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_bt_next_part_numbers_param = bt_next_part_numbers_param;
    let p_cid = cid;
    let p_did = did;

    let uri_str = format!("{}/partnumber/nextnumbers", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<UpdateNextNumbersError> = serde_json::from_str(&content).ok();
